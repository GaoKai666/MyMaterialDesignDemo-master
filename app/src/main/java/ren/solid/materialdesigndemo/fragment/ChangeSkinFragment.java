...
public class ChangeSkinFragment extends BaseFragment {
    ...
    private static String SKIN_RED_NAME = "skin_red.skin";
    ...
    protected void setUpView() {
        ...
        $(R.id.ll_red).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String skinFullName = SKIN_DIR + File.separator + "skin_red.skin";
                FileUtils.moveRawToDir(getMContext(), "skin_red.skin", skinFullName);
                File skin = new File(skinFullName);
                if (!skin.exists()) {
                    Toast.makeText(getMContext(), "请检查" + skinFullName + "是否存在", Toast.LENGTH_SHORT).show();
                    return;
